...
export function FeaturedSlider({
  ...
  return (
    <LocalizedClientLink href={`/products/${product.handle}`} className="block w-full h-full">
      <div className="relative w-full cursor-pointer group" style={{ height: `${height}px` }}>
        <div className="relative w-full h-full bg-gray-50 rounded-lg overflow-hidden">
          ...
          <div className="absolute left-6 bottom-6 bg-[#8B5E4C] text-white rounded-lg p-6 shadow-lg max-w-[85%] md:max-w-[70%]">
            ...
            <p className="text-sm opacity-95 line-clamp-3">{product.description?.slice(0, 150)}</p>
          </div>
          ...
          <div className="flex justify-center gap-2 mt-4">
            {displayImages.map((_, idx) => (
              <button 
                ...
                className={`w-2 h-2 rounded-full transition ${idx === currentImageIndex ? "bg-gray-800" : "bg-gray-300"}`} 
